= 13. Clash, последовательные схемы

В xref:12-clash-intro:index.adoc[предыдущий раз] мы познакомились с тем, как писать комбинационные схемы.
Этого, конечно, недостаточно, по тем же самым причинам, которые мы озвучивали xref:05-sequential-logic:index.adoc[ранее].
...
[, haskell]
.src/12-clash/clash-examples/src/SumReduce.hs
----
...
Это как раз xref:08-automata:index.adoc[функция переходов] stem:[\delta: Q \times \Sigma_{in} \to Q \times (\Sigma_{out})].
...
[, haskell]
.src/12-clash/clash-examples/tests/Tests/SumReduce.hs
----
...
Опять-таки пример, который нам xref:06-bus:exercises.adoc[знаком].

[, haskell]
.src/12-clash/clash-examples/src/Pairwise.hs
----
