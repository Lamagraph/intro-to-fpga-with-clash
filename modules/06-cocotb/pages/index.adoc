...
----
<1> Хоть в SystemVerilog есть классы{empty}footnote:[https://www.chipverify.com/systemverilog/systemverilog-class], но ООП в питоне привычнее.
...
<4> Реализуем ту же функциональность, которую тестируем. В абсолютном большинстве случаев, на питоне это делать приятнее.
