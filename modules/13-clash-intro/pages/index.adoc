...
В xref:12-haskell:index.adoc[прошлый раз] мы уже установили Haskell.
...
[, haskell]
.src/13-clash/clash-examples/src/FullAdder.hs
----
...
[,haskell]
.src/13-clash/clash-examples/tests/Tests/FullAdder.hs
----
...
. Добавить модуль в `src/13-clash/clash-examples/clash-examples.cabal` в `library` и `test-suite`.
. Добавить нужную функцию в `src/13-clash/clash-examples/tests/unittests.hs`.
====
