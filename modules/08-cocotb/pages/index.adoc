...
Как мы увидели в xref:07-bus:index.adoc[прошлый раз], писать тестбенчи на SystemVerilog не очень приятно.
...
Она должна появиться после установки OSS CAD Suite и настройки окружения (`oss-cad-suite/environment`).

Для начала посмотрим на простейший пример.

[, python]
.src/08-cocotb/adder_1_bit/adder_1_bit_tb.py
----
...
[, makefile]
.src/08-cocotb/adder_1_bit/Makefile
----
