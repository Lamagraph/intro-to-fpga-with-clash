...
Тем не менее новые версии проекта выходят редко и не всегда оперативно попадают в репозитории дистрибутивов. При этом исправлений и изменений накапливается достаточноfootnote:[https://github.com/steveicarus/iverilog/discussions/1058], поэтому предлагается воспользоваться пакетом OSS CAD Suite по xref:down-suite.adoc[инструкции].
...
[,console]
----
$ iverilog -g2012 -o build/hello_world hello_world.sv
----
