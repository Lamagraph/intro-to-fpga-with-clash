...
В xref:11-haskell:index.adoc[прошлый раз] мы уже установили Haskell.
...
[, haskell]
.src/12-clash/clash-examples/src/FullAdder.hs
----
...
. Добавить модуль в `src/12-clash/clash-examples/clash-examples.cabal` в `library` и `test-suite`.
. Добавить нужную функцию в `src/12-clash/clash-examples/tests/unittests.hs`.
====
